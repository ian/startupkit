 ...
 */
function checkProjectReferences(
	dir: string
	...
	for (const file of files) {
		...
		if (file.endsWith("package.json")) {
			...
			if (json.name === "PROJECT") {
				console.warn(
					`[post-install-check] Issue found in ${file}: package.json name field is PROJECT`
				)
				issues.push({
					file,
					line: 1,
					content: "package.json name field is PROJECT"
				})
			}
		} else {
			...
			lines.forEach((line, idx) => {
				...
				if (line.includes("PROJECT")) {
					...
					console.warn(
						`[post-install-check] Issue found in ${file}:${idx + 1}: ${line.trim()}`
					)
	...
	console.log(
		`[post-install-check] Finished scanning. Total files checked: ${files.length}`
	)
	return issues
}
...
function runPostInstallCheck({
	dir,
	mode
}: { dir: string; mode: "repo" | "app" }) {
	console.log(
		`[post-install-check] Running post-install check in '${dir}' with mode '${mode}'`
	)
	if (mode === "repo") {
