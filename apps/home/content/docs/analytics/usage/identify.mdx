...
declare function AuthProvider(props: { onIdentify: (user: { id: string; email: string; name: string }) => void; children: React.ReactNode }): JSX.Element
// ---cut---
<AuthProvider
  onIdentify={(user) => {
    identify(user.id, {
...
// @noErrors
import { track } from "@repo/analytics/server"
declare const user: { id: string; email: string; firstName: string; lastName: string }
// ---cut---
await track({
