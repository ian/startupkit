...
export function Weather({
	...
	const currentHigh = Math.max(...result.hourly.temperature_2m.slice(0, 24));
	const currentLow = Math.min(...result.hourly.temperature_2m.slice(0, 24));

	const isDay = isWithinInterval(new Date(result.current.time), {
		start: new Date(result.daily.sunrise[0] as string),
		end: new Date(result.daily.sunset[0] as string),
	});
	...
	const displayTimes = result.hourly.time.slice(
		currentTimeIndex,
		currentTimeIndex + hoursToShow,
	);
	const displayTemperatures = result.hourly.temperature_2m.slice(
		currentTimeIndex,
		currentTimeIndex + hoursToShow,
	);
	...
	return (
		<div
			className={cn(
				"flex flex-col gap-4 rounded-2xl p-4 bg-foreground/10 max-w-[500px]",
				{
					"bg-blue-400": isDay,
				},
				{
				...
				{displayTimes.map((time, index) => (
					<div key={time} className="flex flex-col items-center gap-1">
						...
						<div className="text-blue-50 text-sm">
							{n(displayTemperatures[index] as number)}
